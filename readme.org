...
#+begin_src bash :session none :results verbatim html :post format-ansi(x=*this*) :cache yes
  poetry install --no-dev --no-interaction --ansi
#+end_src
...
Extract the first 100 labels (which are the first two types) for the first class
...
*** Soft SVM
** K-Nearest Neighbor
#+begin_src python :colnames '("label" "petal length" "sepal length")
